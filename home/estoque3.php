...
function setEvent(){
    ...
    function chama_atualiza(){
     ...
     window.location.href = ("estoque3.php");
...
$sql = 'select sum(valor_total_por_item) from estoque_sao_paulo';
$valor_total = $conexao->query( $sql )->fetch();


$sql = 'select sum(qtd_estoque) from estoque_sao_paulo';
...
<div class="tudo">
    ...
    <div class="centralizar">
     <label class="titulo">ESTOQUE 3 - SÃO PAULO</label><br>
    </div>  
    
        <form class="form" action="estoque3.php" method="post">
...
$buscar_cod_produto = $conexao->query( "select cod_produto from estoque_sao_paulo group by cod_produto" );
...
$buscar_descr_produto = $conexao->query( "select descr_produto from estoque_sao_paulo group by descr_produto" );
...
$sql = 'select * from estoque_sao_paulo';
...
$sql = 'select * from estoque_sao_paulo where cod_produto = "'.$_POST['select_cod_produto'].'"';
$todos_produtos = $conexao->query( $sql )->fetch();
$total_de_instalacoes = 0;  
foreach ( $conexao->query( $sql ) as $row ) {         
...
$sql = 'select * from estoque_sao_paulo where descr_produto = "'.$_POST['select_descricao_produto'].'"';
$todos_produtos = $conexao->query( $sql )->fetch();
$total_de_instalacoes = 0;  
foreach ( $conexao->query( $sql ) as $row ) {         
...
echo '</tbody>';
 ...
 <!--   /*********** impressao **********/-->
     ...
     <div id="imprimir_conteudo" class="impressao" style="display: none;"> 
          <label style="font-weight: bold; font-family: arial; font-size: 12px;">ESTOQUE:</label>
<label style="font-weight: bold; font-family: arial; font-size: 12px; color: blue;">SÃO PAULO </label>
...
echo '</tbody>';
        ...
        ?>
          ...
          <script> 
            ...
            a.document.write('<div style="font-size: 12px; text-align: center; text-weight: bold;">');
...
a.document.write('<label style="background: black; color: white; width: 70%; font-family: arial; font-weight: bold; font-size: 20px; padding-top: 3px; padding-bottom: 3px;  ">CONTROLE DE ESTOQUE - ESTOQUE 3 - SÃO PAULO </label>');
 a.document.write('</div>');
                       a.document.write('</div>');
...
<a href="estoque3_inicial.php"><button class="btn_voltar">voltar</button></a>
...
<label style="font-weight: bold;">SÃO PAULO </label><br><br>
