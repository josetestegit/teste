...
function setEvent(){
    ...
    function chama_atualiza(){
     ...
     window.location.href = ("estoque2.php");
...
$sql = 'SELECT sum(valor_total_por_item) from estoque_sao_jose ';
$valor_total = $conexao->query( $sql )->fetch();


$sql = 'select sum(qtd_estoque) from estoque_sao_jose';
...
<div class="tudo">
    ...
    <div class="centralizar">
     <label class="titulo">ESTOQUE 2 - SÃO JOSÉ DOS PINHAIS</label><br>
    </div>  
    
        <form class="form" action="estoque2.php" method="post">
...
$buscar_cod_produto = $conexao->query( "select cod_produto from estoque_sao_jose group by cod_produto" );
...
$buscar_descr_produto = $conexao->query( "select descr_produto from estoque_sao_jose group by descr_produto" );
...
<tbody>
    ...
    if (isset($_POST['select_cod_produto'], 
              $_POST['select_descricao_produto'] 
             )){
...
if($_POST['select_cod_produto'] == '' && $_POST['select_descricao_produto'] == ''   ){
...
if($_POST['select_cod_produto'] != ''){



$sql = 'select * from estoque_sao_jose where cod_produto = "'.$_POST['select_cod_produto'].'"';
$todos_produtos = $conexao->query( $sql )->fetch();
$total_de_instalacoes = 0;  
foreach ( $conexao->query( $sql ) as $row ) {         
 $preco_medio = $row['preco_medio']; 
...
if($_POST['select_descricao_produto'] != ''){



$sql = 'select * from estoque_sao_jose where descr_produto = "'.$_POST['select_descricao_produto'].'"';
$todos_produtos = $conexao->query( $sql )->fetch();
$total_de_instalacoes = 0;  
foreach ( $conexao->query( $sql ) as $row ) {         
...
<label style="font-weight: bold; font-family: arial; font-size: 12px; color: blue;">SÃO JOSÉ DOS PINHAIS </label>
...
$sql = 'select * from estoque_agua_verde where cod_produto = "'.$_POST['select_cod_produto'].'"';
$todos_produtos = $conexao->query( $sql )->fetch();
$total_de_instalacoes = 0;  
foreach ( $conexao->query( $sql ) as $row ) {         
...
$sql = 'select * from estoque_agua_verde where descr_produto = "'.$_POST['select_descricao_produto'].'"';
$todos_produtos = $conexao->query( $sql )->fetch();
$total_de_instalacoes = 0;  
foreach ( $conexao->query( $sql ) as $row ) {         
...
a.document.write('<label style="background: black; color: white; width: 70%; font-family: arial; font-weight: bold; font-size: 20px; padding-top: 3px; padding-bottom: 3px;  ">CONTROLE DE ESTOQUE - ESTOQUE 2 - SÃO JOSÉ </label>');
 a.document.write('</div>');
    a.document.write('</div>');
...
<a href="estoque2_inicial.php"><button class="btn_voltar">voltar</button></a>
...
<label style="font-weight: bold;">SÃO JOSÉ DOS PINHAIS</label> <br><br> 
